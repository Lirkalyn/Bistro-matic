...
char *el_sumator(char *num1, char *num2)
{
    ...
    if (l1 < l2) {
        ...
        for (int i = l1 - 1 ; i != l2 ; i++)
            tmp_num[i] = '0';
        my_revstr(tmp_num);
        return (sum_result(num1, num2, addsum(tmp_num, num2, result)));
    } else if (l2 < l1) {
        result = malloc(sizeof(num1) + 2);
        ...
        for (int i = l2 - 1 ; i != l1 ; i++)
            tmp_num[i] = '0';
        my_revstr(tmp_num);
        return (sum_result(num1, num2, addsum(tmp_num, num1, result)));
    } else {
        result = malloc(sizeof(num1) + 2);
        return (sum_result(num1, num2, addsum(num1, num2, result)));
    }
}
